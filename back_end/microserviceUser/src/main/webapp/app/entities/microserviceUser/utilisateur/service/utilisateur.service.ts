...
export class UtilisateurService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/utilisateurs', 'microserviceuser');
  ...
  addUtilisateurToCollectionIfMissing<Type extends Pick<IUtilisateur, 'id'>>(
    ...
    if (utilisateurs.length > 0) {
      const utilisateurCollectionIdentifiers = utilisateurCollection.map(
        utilisateurItem => this.getUtilisateurIdentifier(utilisateurItem)!,
      );
