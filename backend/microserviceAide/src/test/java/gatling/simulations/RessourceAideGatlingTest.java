...
/**
 * Performance test for the RessourceAide entity.
 */
public class RessourceAideGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all ressourceAides")
                    .get("/services/microserviceaide/api/ressource-aides")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ressourceAide")
                        .post("/services/microserviceaide/api/ressource-aides")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nom\": \"SAMPLE_TEXT\"" + ", \"libelle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ressourceAide_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created ressourceAide")
                            .get("/services/microserviceaide${new_ressourceAide_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created ressourceAide")
                        .delete("/services/microserviceaide${new_ressourceAide_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the RessourceAide entity").exec(scn);
