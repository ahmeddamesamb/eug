...
public class CandidatGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8090");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all candidats")
                    .get("/services/microserviceaclc/api/candidats")
                ...
                .exec(
                    http("Create new candidat")
                        .post("/services/microserviceaclc/api/candidats")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created candidat")
                            .get("/services/microserviceaclc${new_candidat_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created candidat")
                        .delete("/services/microserviceaclc${new_candidat_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
