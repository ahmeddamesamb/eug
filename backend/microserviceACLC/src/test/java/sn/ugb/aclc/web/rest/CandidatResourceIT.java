...
class CandidatResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/candidats/_search";
    ...
    @Autowired
    private CandidatSearchRepository candidatSearchRepository;
    ...
    public void cleanupElasticSearchRepository() {
        candidatSearchRepository.deleteAll();
        assertThat(candidatSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createCandidat() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(candidatSearchRepository.findAll());
        // Create the Candidat
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(candidatSearchRepository.findAll());
    ...
    void putExistingCandidat() throws Exception {
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(candidatSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<Candidat> candidatSearchList = IterableUtils.toList(candidatSearchRepository.findAll());
                Candidat testCandidatSearch = candidatSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testCandidatSearch.getNom()).isEqualTo(UPDATED_NOM);
                assertThat(testCandidatSearch.getPrenom()).isEqualTo(UPDATED_PRENOM);
                assertThat(testCandidatSearch.getDateNaissance()).isEqualTo(UPDATED_DATE_NAISSANCE);
                assertThat(testCandidatSearch.getEmail()).isEqualTo(UPDATED_EMAIL);
            });
    }
    ...
    @Test
    @Transactional
    void searchCandidat() throws Exception {
        // Initialize the database
        candidat = candidatRepository.saveAndFlush(candidat);
        candidatSearchRepository.save(candidat);

        // Search the candidat
        restCandidatMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + candidat.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(candidat.getId().intValue())))
            .andExpect(jsonPath("$.[*].nom").value(hasItem(DEFAULT_NOM)))
