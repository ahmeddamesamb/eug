...
export class CandidatService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/candidats', 'microserviceaclc');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/candidats/_search', 'microserviceaclc');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestCandidat[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),
      catchError(() => scheduled([new HttpResponse<ICandidat[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends ICandidat | NewCandidat | PartialUpdateCandidat>(candidat: T): RestOf<T> {
    return {
      ...candidat,
      dateNaissance: candidat.dateNaissance?.format(DATE_FORMAT) ?? null,
    };
  }
