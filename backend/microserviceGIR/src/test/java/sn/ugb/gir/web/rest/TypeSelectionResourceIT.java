...
class TypeSelectionResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/type-selections/_search";
    ...
    @Autowired
    private TypeSelectionSearchRepository typeSelectionSearchRepository;
    ...
    public void cleanupElasticSearchRepository() {
        typeSelectionSearchRepository.deleteAll();
        assertThat(typeSelectionSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createTypeSelection() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(typeSelectionSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(typeSelectionSearchRepository.findAll());
    ...
    void putExistingTypeSelection() throws Exception {
        ...
        typeSelectionSearchRepository.save(typeSelection);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(typeSelectionSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(typeSelectionSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<TypeSelection> typeSelectionSearchList = IterableUtils.toList(typeSelectionSearchRepository.findAll());
                TypeSelection testTypeSelectionSearch = typeSelectionSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testTypeSelectionSearch.getLibelleTypeSelection()).isEqualTo(UPDATED_LIBELLE_TYPE_SELECTION);
            });
    }
    ...
    @Test
    @Transactional
    void searchTypeSelection() throws Exception {
        // Initialize the database
        typeSelection = typeSelectionRepository.saveAndFlush(typeSelection);
        typeSelectionSearchRepository.save(typeSelection);

        // Search the typeSelection
        restTypeSelectionMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + typeSelection.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(typeSelection.getId().intValue())))
            .andExpect(jsonPath("$.[*].libelleTypeSelection").value(hasItem(DEFAULT_LIBELLE_TYPE_SELECTION)));
