...
class DisciplineSportiveResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/discipline-sportives/_search";
    ...
    @Autowired
    private DisciplineSportiveSearchRepository disciplineSportiveSearchRepository;
    ...
    public void cleanupElasticSearchRepository() {
        disciplineSportiveSearchRepository.deleteAll();
        assertThat(disciplineSportiveSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createDisciplineSportive() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(disciplineSportiveSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(disciplineSportiveSearchRepository.findAll());
    ...
    void putExistingDisciplineSportive() throws Exception {
        ...
        disciplineSportiveSearchRepository.save(disciplineSportive);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(disciplineSportiveSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(disciplineSportiveSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<DisciplineSportive> disciplineSportiveSearchList = IterableUtils.toList(disciplineSportiveSearchRepository.findAll());
                DisciplineSportive testDisciplineSportiveSearch = disciplineSportiveSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testDisciplineSportiveSearch.getLibelleDisciplineSportive()).isEqualTo(UPDATED_LIBELLE_DISCIPLINE_SPORTIVE);
            });
    }
    ...
    @Test
    @Transactional
    void searchDisciplineSportive() throws Exception {
        // Initialize the database
        disciplineSportive = disciplineSportiveRepository.saveAndFlush(disciplineSportive);
        disciplineSportiveSearchRepository.save(disciplineSportive);

        // Search the disciplineSportive
        restDisciplineSportiveMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + disciplineSportive.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(disciplineSportive.getId().intValue())))
            .andExpect(jsonPath("$.[*].libelleDisciplineSportive").value(hasItem(DEFAULT_LIBELLE_DISCIPLINE_SPORTIVE)));
