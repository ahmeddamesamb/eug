        ...
        .on(
            exec(
                http("Get all regions")
                    .get("/services/microservicegir/api/regions")
                ...
                .exec(
                    http("Create new region")
                        .post("/services/microservicegir/api/regions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleRegion\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .repeat(5)
                .on(
                    exec(http("Get created region").get("/services/microservicegir${new_region_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created region").delete("/services/microservicegir${new_region_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );
