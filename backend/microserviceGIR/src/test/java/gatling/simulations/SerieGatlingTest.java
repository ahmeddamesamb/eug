 ...
 */
public class SerieGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all series")
                    .get("/services/microservicegir/api/series")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new serie")
                        .post("/services/microservicegir/api/series")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"codeSerie\": \"SAMPLE_TEXT\"" +
                                ", \"libelleSerie\": \"SAMPLE_TEXT\"" +
                                ", \"sigleSerie\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .repeat(5)
                .on(
                    exec(http("Get created serie").get("/services/microservicegir${new_serie_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created serie").delete("/services/microservicegir${new_serie_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Serie entity").exec(scn);
