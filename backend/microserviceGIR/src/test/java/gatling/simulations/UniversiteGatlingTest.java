...
/**
 * Performance test for the Universite entity.
 */
public class UniversiteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all universites")
                    .get("/services/microservicegir/api/universites")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new universite")
                        .post("/services/microservicegir/api/universites")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomUniversite\": \"SAMPLE_TEXT\"" +
                                ", \"sigleUniversite\": \"SAMPLE_TEXT\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_universite_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created universite")
                            .get("/services/microservicegir${new_universite_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created universite")
                        .delete("/services/microservicegir${new_universite_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Universite entity").exec(scn);
