...
/**
 * Performance test for the Mention entity.
 */
public class MentionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all mentions")
                    .get("/services/microservicegir/api/mentions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new mention")
                        .post("/services/microservicegir/api/mentions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleMention\": \"SAMPLE_TEXT\"" + ", \"actifYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_mention_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created mention").get("/services/microservicegir${new_mention_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created mention").delete("/services/microservicegir${new_mention_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Mention entity").exec(scn);
