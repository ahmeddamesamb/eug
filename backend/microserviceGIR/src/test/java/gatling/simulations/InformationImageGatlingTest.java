...
/**
 * Performance test for the InformationImage entity.
 */
public class InformationImageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all informationImages")
                    .get("/services/microservicegir/api/information-images")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new informationImage")
                        .post("/services/microservicegir/api/information-images")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"description\": \"SAMPLE_TEXT\"" +
                                ", \"cheminPath\": \"SAMPLE_TEXT\"" +
                                ", \"cheminFile\": \"SAMPLE_TEXT\"" +
                                ", \"enCoursYN\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_informationImage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created informationImage")
                            .get("/services/microservicegir${new_informationImage_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created informationImage")
                        .delete("/services/microservicegir${new_informationImage_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InformationImage entity").exec(scn);
