...
/**
 * Performance test for the FormationInvalide entity.
 */
public class FormationInvalideGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all formationInvalides")
                    .get("/services/microservicegir/api/formation-invalides")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formationInvalide")
                        .post("/services/microservicegir/api/formation-invalides")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"actifYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formationInvalide_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created formationInvalide")
                            .get("/services/microservicegir${new_formationInvalide_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created formationInvalide")
                        .delete("/services/microservicegir${new_formationInvalide_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormationInvalide entity").exec(scn);
