...
/**
 * Performance test for the FormationPrivee entity.
 */
public class FormationPriveeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all formationPrivees")
                    .get("/services/microservicegir/api/formation-privees")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formationPrivee")
                        .post("/services/microservicegir/api/formation-privees")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nombreMensualites\": 0" +
                                ", \"paiementPremierMoisYN\": null" +
                                ", \"paiementDernierMoisYN\": null" +
                                ", \"fraisDossierYN\": null" +
                                ", \"coutTotal\": 0" +
                                ", \"mensualite\": 0" +
                                ", \"actifYN\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formationPrivee_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created formationPrivee")
                            .get("/services/microservicegir${new_formationPrivee_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created formationPrivee")
                        .delete("/services/microservicegir${new_formationPrivee_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormationPrivee entity").exec(scn);
