 ...
 */
public class NiveauGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all niveaus")
                    .get("/services/microservicegir/api/niveaus")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new niveau")
                        .post("/services/microservicegir/api/niveaus")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleNiveau\": \"SAMPLE_TEXT\"" +
                                ", \"codeNiveau\": \"SAMPLE_TEXT\"" +
                                ", \"anneeEtude\": \"SAMPLE_TEXT\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_niveau_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created niveau").get("/services/microservicegir${new_niveau_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created niveau").delete("/services/microservicegir${new_niveau_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Niveau entity").exec(scn);
