...
/**
 * Performance test for the PaiementFormationPrivee entity.
 */
public class PaiementFormationPriveeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all paiementFormationPrivees")
                    .get("/services/microservicegir/api/paiement-formation-privees")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paiementFormationPrivee")
                        .post("/services/microservicegir/api/paiement-formation-privees")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"datePaiement\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"moisPaiement\": \"SAMPLE_TEXT\"" +
                                ", \"anneePaiement\": \"SAMPLE_TEXT\"" +
                                ", \"payerMensualiteYN\": null" +
                                ", \"payerDelaisYN\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paiementFormationPrivee_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created paiementFormationPrivee")
                            .get("/services/microservicegir${new_paiementFormationPrivee_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created paiementFormationPrivee")
                        .delete("/services/microservicegir${new_paiementFormationPrivee_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaiementFormationPrivee entity").exec(scn);
