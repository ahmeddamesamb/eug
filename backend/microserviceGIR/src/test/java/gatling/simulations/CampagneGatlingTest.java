...
/**
 * Performance test for the Campagne entity.
 */
public class CampagneGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all campagnes")
                    .get("/services/microservicegir/api/campagnes")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new campagne")
                        .post("/services/microservicegir/api/campagnes")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleCampagne\": \"SAMPLE_TEXT\"" +
                                ", \"dateDebut\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateFin\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"libelleAbrege\": \"SAMPLE_TEXT\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_campagne_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created campagne").get("/services/microservicegir${new_campagne_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created campagne")
                        .delete("/services/microservicegir${new_campagne_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Campagne entity").exec(scn);
