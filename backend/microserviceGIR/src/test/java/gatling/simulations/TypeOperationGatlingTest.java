 ...
 */
public class TypeOperationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeOperations")
                    .get("/services/microservicegir/api/type-operations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeOperation")
                        .post("/services/microservicegir/api/type-operations")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeOperation\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeOperation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeOperation")
                            .get("/services/microservicegir${new_typeOperation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeOperation")
                        .delete("/services/microservicegir${new_typeOperation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeOperation entity").exec(scn);
