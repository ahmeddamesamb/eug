...
/**
 * Performance test for the TypeBourse entity.
 */
public class TypeBourseGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeBourses")
                    .get("/services/microservicegir/api/type-bourses")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeBourse")
                        .post("/services/microservicegir/api/type-bourses")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeBourse\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeBourse_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeBourse")
                            .get("/services/microservicegir${new_typeBourse_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeBourse")
                        .delete("/services/microservicegir${new_typeBourse_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeBourse entity").exec(scn);
