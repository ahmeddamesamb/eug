...
/**
 * Performance test for the InformationPersonnelle entity.
 */
public class InformationPersonnelleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all informationPersonnelles")
                    .get("/services/microservicegir/api/information-personnelles")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new informationPersonnelle")
                        .post("/services/microservicegir/api/information-personnelles")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomEtu\": \"SAMPLE_TEXT\"" +
                                ", \"nomJeuneFilleEtu\": \"SAMPLE_TEXT\"" +
                                ", \"prenomEtu\": \"SAMPLE_TEXT\"" +
                                ", \"statutMarital\": \"SAMPLE_TEXT\"" +
                                ", \"regime\": 0" +
                                ", \"profession\": \"SAMPLE_TEXT\"" +
                                ", \"adresseEtu\": \"SAMPLE_TEXT\"" +
                                ", \"telEtu\": \"SAMPLE_TEXT\"" +
                                ", \"emailEtu\": \"SAMPLE_TEXT\"" +
                                ", \"adresseParent\": \"SAMPLE_TEXT\"" +
                                ", \"telParent\": \"SAMPLE_TEXT\"" +
                                ", \"emailParent\": \"SAMPLE_TEXT\"" +
                                ", \"nomParent\": \"SAMPLE_TEXT\"" +
                                ", \"prenomParent\": \"SAMPLE_TEXT\"" +
                                ", \"handicapYN\": null" +
                                ", \"photo\": \"SAMPLE_TEXT\"" +
                                ", \"ordiPersoYN\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_informationPersonnelle_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created informationPersonnelle")
                            .get("/services/microservicegir${new_informationPersonnelle_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created informationPersonnelle")
                        .delete("/services/microservicegir${new_informationPersonnelle_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InformationPersonnelle entity").exec(scn);
