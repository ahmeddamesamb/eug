...
/**
 * Performance test for the Cycle entity.
 */
public class CycleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all cycles")
                    .get("/services/microservicegir/api/cycles")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new cycle")
                        .post("/services/microservicegir/api/cycles")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleCycle\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_cycle_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created cycle").get("/services/microservicegir${new_cycle_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created cycle").delete("/services/microservicegir${new_cycle_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Cycle entity").exec(scn);
