...
/**
 * Performance test for the TypeSelection entity.
 */
public class TypeSelectionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeSelections")
                    .get("/services/microservicegir/api/type-selections")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeSelection")
                        .post("/services/microservicegir/api/type-selections")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeSelection\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeSelection_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeSelection")
                            .get("/services/microservicegir${new_typeSelection_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeSelection")
                        .delete("/services/microservicegir${new_typeSelection_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeSelection entity").exec(scn);
