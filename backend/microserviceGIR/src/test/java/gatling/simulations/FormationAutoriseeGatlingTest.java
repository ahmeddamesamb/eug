...
/**
 * Performance test for the FormationAutorisee entity.
 */
public class FormationAutoriseeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all formationAutorisees")
                    .get("/services/microservicegir/api/formation-autorisees")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formationAutorisee")
                        .post("/services/microservicegir/api/formation-autorisees")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formationAutorisee_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created formationAutorisee")
                            .get("/services/microservicegir${new_formationAutorisee_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created formationAutorisee")
                        .delete("/services/microservicegir${new_formationAutorisee_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the FormationAutorisee entity").exec(scn);
