 ...
 */
public class PaysGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all pays").get("/services/microservicegir/api/pays").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pays")
                        .post("/services/microservicegir/api/pays")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libellePays\": \"SAMPLE_TEXT\"" +
                                ", \"paysEnAnglais\": \"SAMPLE_TEXT\"" +
                                ", \"nationalite\": \"SAMPLE_TEXT\"" +
                                ", \"codePays\": \"SAMPLE_TEXT\"" +
                                ", \"uEMOAYN\": null" +
                                ", \"cEDEAOYN\": null" +
                                ", \"rIMYN\": null" +
                                ", \"autreYN\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_pays_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created pays").get("/services/microservicegir${new_pays_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created pays").delete("/services/microservicegir${new_pays_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Pays entity").exec(scn);
