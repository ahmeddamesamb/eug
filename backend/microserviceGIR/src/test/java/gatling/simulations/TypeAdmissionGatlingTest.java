...
/**
 * Performance test for the TypeAdmission entity.
 */
public class TypeAdmissionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeAdmissions")
                    .get("/services/microservicegir/api/type-admissions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeAdmission")
                        .post("/services/microservicegir/api/type-admissions")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeAdmission\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeAdmission_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeAdmission")
                            .get("/services/microservicegir${new_typeAdmission_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeAdmission")
                        .delete("/services/microservicegir${new_typeAdmission_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeAdmission entity").exec(scn);
