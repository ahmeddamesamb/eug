...
/**
 * Performance test for the DisciplineSportive entity.
 */
public class DisciplineSportiveGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all disciplineSportives")
                    .get("/services/microservicegir/api/discipline-sportives")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new disciplineSportive")
                        .post("/services/microservicegir/api/discipline-sportives")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleDisciplineSportive\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_disciplineSportive_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created disciplineSportive")
                            .get("/services/microservicegir${new_disciplineSportive_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created disciplineSportive")
                        .delete("/services/microservicegir${new_disciplineSportive_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DisciplineSportive entity").exec(scn);
