...
/**
 * Performance test for the Doctorat entity.
 */
public class DoctoratGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all doctorats")
                    .get("/services/microservicegir/api/doctorats")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new doctorat")
                        .post("/services/microservicegir/api/doctorats")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"sujet\": \"SAMPLE_TEXT\"" +
                                ", \"anneeInscriptionDoctorat\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"encadreurId\": 0" +
                                ", \"laboratoirId\": 0" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_doctorat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created doctorat").get("/services/microservicegir${new_doctorat_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created doctorat")
                        .delete("/services/microservicegir${new_doctorat_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Doctorat entity").exec(scn);
