...
/**
 * Performance test for the AnneeAcademique entity.
 */
public class AnneeAcademiqueGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all anneeAcademiques")
                    .get("/services/microservicegir/api/annee-academiques")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new anneeAcademique")
                        .post("/services/microservicegir/api/annee-academiques")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleAnneeAcademique\": \"SAMPLE_TEXT\"" +
                                ", \"anneeAc\": 0" +
                                ", \"separateur\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_anneeAcademique_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created anneeAcademique")
                            .get("/services/microservicegir${new_anneeAcademique_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created anneeAcademique")
                        .delete("/services/microservicegir${new_anneeAcademique_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AnneeAcademique entity").exec(scn);
