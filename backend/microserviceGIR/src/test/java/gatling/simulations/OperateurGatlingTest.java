 ...
 */
public class OperateurGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all operateurs")
                    .get("/services/microservicegir/api/operateurs")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new operateur")
                        .post("/services/microservicegir/api/operateurs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleOperateur\": \"SAMPLE_TEXT\"" +
                                ", \"userLogin\": \"SAMPLE_TEXT\"" +
                                ", \"codeOperateur\": \"SAMPLE_TEXT\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_operateur_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created operateur")
                            .get("/services/microservicegir${new_operateur_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created operateur")
                        .delete("/services/microservicegir${new_operateur_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Operateur entity").exec(scn);
