...
/**
 * Performance test for the ProgrammationInscription entity.
 */
public class ProgrammationInscriptionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all programmationInscriptions")
                    .get("/services/microservicegir/api/programmation-inscriptions")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new programmationInscription")
                        .post("/services/microservicegir/api/programmation-inscriptions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleProgrammation\": \"SAMPLE_TEXT\"" +
                                ", \"dateDebutProgrammation\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateFinProgrammation\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"ouvertYN\": null" +
                                ", \"emailUser\": \"SAMPLE_TEXT\"" +
                                ", \"dateForclosClasse\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"forclosClasseYN\": null" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_programmationInscription_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created programmationInscription")
                            .get("/services/microservicegir${new_programmationInscription_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created programmationInscription")
                        .delete("/services/microservicegir${new_programmationInscription_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ProgrammationInscription entity").exec(scn);
