...
public class ZoneGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all zones").get("/services/microservicegir/api/zones").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new zone")
                        .post("/services/microservicegir/api/zones")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleZone\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                ...
                .repeat(5)
                .on(
                    exec(http("Get created zone").get("/services/microservicegir${new_zone_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created zone").delete("/services/microservicegir${new_zone_url}").headers(headers_http_authenticated))
                .pause(10)
        );
