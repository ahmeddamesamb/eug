...
/**
 * Performance test for the Baccalaureat entity.
 */
public class BaccalaureatGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all baccalaureats")
                    .get("/services/microservicegir/api/baccalaureats")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new baccalaureat")
                        .post("/services/microservicegir/api/baccalaureats")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"origineScolaire\": \"SAMPLE_TEXT\"" +
                                ", \"anneeBac\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"numeroTable\": 0" +
                                ", \"natureBac\": \"SAMPLE_TEXT\"" +
                                ", \"mentionBac\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_baccalaureat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created baccalaureat")
                            .get("/services/microservicegir${new_baccalaureat_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created baccalaureat")
                        .delete("/services/microservicegir${new_baccalaureat_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Baccalaureat entity").exec(scn);
