...
public class OperationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all operations")
                    .get("/services/microservicegir/api/operations")
                ...
                .exec(
                    http("Create new operation")
                        .post("/services/microservicegir/api/operations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"dateExecution\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"emailUser\": \"SAMPLE_TEXT\"" +
                                ", \"detailOperation\": null" +
                                ", \"infoSysteme\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                ...
                .on(
                    exec(
                        http("Get created operation")
                            .get("/services/microservicegir${new_operation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created operation")
                        .delete("/services/microservicegir${new_operation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );
