 ...
 */
public class DepartementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all departements")
                    .get("/services/microservicegir/api/departements")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new departement")
                        .post("/services/microservicegir/api/departements")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nomDepatement\": \"SAMPLE_TEXT\"" + ", \"actifYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_departement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created departement")
                            .get("/services/microservicegir${new_departement_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created departement")
                        .delete("/services/microservicegir${new_departement_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Departement entity").exec(scn);
