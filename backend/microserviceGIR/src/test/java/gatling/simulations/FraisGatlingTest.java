...
/**
 * Performance test for the Frais entity.
 */
public class FraisGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all frais").get("/services/microservicegir/api/frais").headers(headers_http_authenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new frais")
                        .post("/services/microservicegir/api/frais")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"valeurFrais\": 0" +
                                ", \"descriptionFrais\": \"SAMPLE_TEXT\"" +
                                ", \"fraisPourAssimileYN\": null" +
                                ", \"fraisPourExonererYN\": null" +
                                ", \"dia\": 0" +
                                ", \"dip\": 0" +
                                ", \"fraisPrivee\": 0" +
                                ", \"dateApplication\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateFin\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"estEnApplicationYN\": null" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_frais_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created frais").get("/services/microservicegir${new_frais_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created frais").delete("/services/microservicegir${new_frais_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Frais entity").exec(scn);
