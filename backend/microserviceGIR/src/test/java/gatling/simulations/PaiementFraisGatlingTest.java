...
/**
 * Performance test for the PaiementFrais entity.
 */
public class PaiementFraisGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all paiementFrais")
                    .get("/services/microservicegir/api/paiement-frais")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paiementFrais")
                        .post("/services/microservicegir/api/paiement-frais")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"obligatoireYN\": null" +
                                ", \"echeancePayeeYN\": null" +
                                ", \"emailUser\": \"SAMPLE_TEXT\"" +
                                ", \"dateForclos\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"forclosYN\": null" +
                                ", \"paimentDelaiYN\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paiementFrais_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created paiementFrais")
                            .get("/services/microservicegir${new_paiementFrais_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created paiementFrais")
                        .delete("/services/microservicegir${new_paiementFrais_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaiementFrais entity").exec(scn);
