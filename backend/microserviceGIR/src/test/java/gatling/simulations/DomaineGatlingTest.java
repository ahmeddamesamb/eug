...
/**
 * Performance test for the Domaine entity.
 */
public class DomaineGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all domaines")
                    .get("/services/microservicegir/api/domaines")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new domaine")
                        .post("/services/microservicegir/api/domaines")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleDomaine\": \"SAMPLE_TEXT\"" + ", \"actifYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_domaine_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created domaine").get("/services/microservicegir${new_domaine_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created domaine").delete("/services/microservicegir${new_domaine_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Domaine entity").exec(scn);
