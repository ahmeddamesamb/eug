...
/**
 * Performance test for the DisciplineSportiveEtudiant entity.
 */
public class DisciplineSportiveEtudiantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all disciplineSportiveEtudiants")
                    .get("/services/microservicegir/api/discipline-sportive-etudiants")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new disciplineSportiveEtudiant")
                        .post("/services/microservicegir/api/discipline-sportive-etudiants")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"licenceSportiveYN\": null" + ", \"competitionUGBYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_disciplineSportiveEtudiant_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created disciplineSportiveEtudiant")
                            .get("/services/microservicegir${new_disciplineSportiveEtudiant_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created disciplineSportiveEtudiant")
                        .delete("/services/microservicegir${new_disciplineSportiveEtudiant_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DisciplineSportiveEtudiant entity").exec(scn);
