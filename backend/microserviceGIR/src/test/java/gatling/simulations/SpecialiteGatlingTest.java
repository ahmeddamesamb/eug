 ...
 */
public class SpecialiteGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all specialites")
                    .get("/services/microservicegir/api/specialites")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new specialite")
                        .post("/services/microservicegir/api/specialites")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomSpecialites\": \"SAMPLE_TEXT\"" +
                                ", \"sigleSpecialites\": \"SAMPLE_TEXT\"" +
                                ", \"specialiteParticulierYN\": null" +
                                ", \"specialitesPayanteYN\": null" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_specialite_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created specialite")
                            .get("/services/microservicegir${new_specialite_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created specialite")
                        .delete("/services/microservicegir${new_specialite_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Specialite entity").exec(scn);
