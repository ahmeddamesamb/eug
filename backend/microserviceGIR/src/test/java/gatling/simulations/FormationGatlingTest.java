 ...
 */
public class FormationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all formations")
                    .get("/services/microservicegir/api/formations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new formation")
                        .post("/services/microservicegir/api/formations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"classeDiplomanteYN\": null" +
                                ", \"libelleDiplome\": \"SAMPLE_TEXT\"" +
                                ", \"codeFormation\": \"SAMPLE_TEXT\"" +
                                ", \"nbreCreditsMin\": 0" +
                                ", \"estParcoursYN\": null" +
                                ", \"lmdYN\": null" +
                                ", \"actifYN\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_formation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created formation")
                            .get("/services/microservicegir${new_formation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created formation")
                        .delete("/services/microservicegir${new_formation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Formation entity").exec(scn);
