...
/**
 * Performance test for the InscriptionAdministrativeFormation entity.
 */
public class InscriptionAdministrativeFormationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all inscriptionAdministrativeFormations")
                    .get("/services/microservicegir/api/inscription-administrative-formations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new inscriptionAdministrativeFormation")
                        .post("/services/microservicegir/api/inscription-administrative-formations")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"inscriptionPrincipaleYN\": null" +
                                ", \"inscriptionAnnuleeYN\": null" +
                                ", \"exonereYN\": null" +
                                ", \"paiementFraisOblYN\": null" +
                                ", \"paiementFraisIntegergYN\": null" +
                                ", \"certificatDelivreYN\": null" +
                                ", \"dateChoixFormation\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateValidationInscription\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_inscriptionAdministrativeFormation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created inscriptionAdministrativeFormation")
                            .get("/services/microservicegir${new_inscriptionAdministrativeFormation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created inscriptionAdministrativeFormation")
                        .delete("/services/microservicegir${new_inscriptionAdministrativeFormation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InscriptionAdministrativeFormation entity").exec(scn);
