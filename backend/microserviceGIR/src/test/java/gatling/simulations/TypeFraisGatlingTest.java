...
/**
 * Performance test for the TypeFrais entity.
 */
public class TypeFraisGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeFrais")
                    .get("/services/microservicegir/api/type-frais")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeFrais")
                        .post("/services/microservicegir/api/type-frais")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeFrais\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeFrais_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeFrais")
                            .get("/services/microservicegir${new_typeFrais_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeFrais")
                        .delete("/services/microservicegir${new_typeFrais_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeFrais entity").exec(scn);
