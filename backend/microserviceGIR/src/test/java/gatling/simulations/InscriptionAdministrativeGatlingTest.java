...
/**
 * Performance test for the InscriptionAdministrative entity.
 */
public class InscriptionAdministrativeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all inscriptionAdministratives")
                    .get("/services/microservicegir/api/inscription-administratives")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new inscriptionAdministrative")
                        .post("/services/microservicegir/api/inscription-administratives")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nouveauInscritYN\": null" +
                                ", \"repriseYN\": null" +
                                ", \"autoriseYN\": null" +
                                ", \"ordreInscription\": 0" +
                                "}"
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_inscriptionAdministrative_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created inscriptionAdministrative")
                            .get("/services/microservicegir${new_inscriptionAdministrative_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created inscriptionAdministrative")
                        .delete("/services/microservicegir${new_inscriptionAdministrative_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InscriptionAdministrative entity").exec(scn);
