...
/**
 * Performance test for the Ministere entity.
 */
public class MinistereGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all ministeres")
                    .get("/services/microservicegir/api/ministeres")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ministere")
                        .post("/services/microservicegir/api/ministeres")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomMinistere\": \"SAMPLE_TEXT\"" +
                                ", \"sigleMinistere\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ministere_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created ministere")
                            .get("/services/microservicegir${new_ministere_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created ministere")
                        .delete("/services/microservicegir${new_ministere_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ministere entity").exec(scn);
