...
/**
 * Performance test for the InscriptionDoctorat entity.
 */
public class InscriptionDoctoratGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all inscriptionDoctorats")
                    .get("/services/microservicegir/api/inscription-doctorats")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new inscriptionDoctorat")
                        .post("/services/microservicegir/api/inscription-doctorats")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"sourceFinancement\": \"SAMPLE_TEXT\"" +
                                ", \"coEncadreurId\": \"SAMPLE_TEXT\"" +
                                ", \"nombreInscription\": 0" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_inscriptionDoctorat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created inscriptionDoctorat")
                            .get("/services/microservicegir${new_inscriptionDoctorat_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created inscriptionDoctorat")
                        .delete("/services/microservicegir${new_inscriptionDoctorat_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InscriptionDoctorat entity").exec(scn);
