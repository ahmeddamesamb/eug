 ...
 */
public class TypeFormationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeFormations")
                    .get("/services/microservicegir/api/type-formations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeFormation")
                        .post("/services/microservicegir/api/type-formations")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeFormation\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeFormation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeFormation")
                            .get("/services/microservicegir${new_typeFormation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeFormation")
                        .delete("/services/microservicegir${new_typeFormation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeFormation entity").exec(scn);
