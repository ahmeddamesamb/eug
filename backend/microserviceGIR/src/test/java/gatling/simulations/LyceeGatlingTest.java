...
/**
 * Performance test for the Lycee entity.
 */
public class LyceeGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all lycees")
                    .get("/services/microservicegir/api/lycees")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new lycee")
                        .post("/services/microservicegir/api/lycees")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"nomLycee\": \"SAMPLE_TEXT\"" +
                                ", \"codeLycee\": \"SAMPLE_TEXT\"" +
                                ", \"villeLycee\": \"SAMPLE_TEXT\"" +
                                ", \"academieLycee\": 0" +
                                ", \"centreExamen\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_lycee_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created lycee").get("/services/microservicegir${new_lycee_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created lycee").delete("/services/microservicegir${new_lycee_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Lycee entity").exec(scn);
