 ...
 */
public class TypeHandicapGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeHandicaps")
                    .get("/services/microservicegir/api/type-handicaps")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeHandicap")
                        .post("/services/microservicegir/api/type-handicaps")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeHandicap\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeHandicap_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeHandicap")
                            .get("/services/microservicegir${new_typeHandicap_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeHandicap")
                        .delete("/services/microservicegir${new_typeHandicap_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeHandicap entity").exec(scn);
