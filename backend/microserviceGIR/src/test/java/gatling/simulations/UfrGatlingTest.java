...
/**
 * Performance test for the Ufr entity.
 */
public class UfrGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all ufrs").get("/services/microservicegir/api/ufrs").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ufr")
                        .post("/services/microservicegir/api/ufrs")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleUfr\": \"SAMPLE_TEXT\"" +
                                ", \"sigleUfr\": \"SAMPLE_TEXT\"" +
                                ", \"prefixe\": \"SAMPLE_TEXT\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ufr_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created ufr").get("/services/microservicegir${new_ufr_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created ufr").delete("/services/microservicegir${new_ufr_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ufr entity").exec(scn);
