 ...
 */
public class EtudiantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all etudiants")
                    .get("/services/microservicegir/api/etudiants")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new etudiant")
                        .post("/services/microservicegir/api/etudiants")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"ine\": \"SAMPLE_TEXT\"" +
                                ", \"codeBU\": \"SAMPLE_TEXT\"" +
                                ", \"emailUGB\": \"SAMPLE_TEXT\"" +
                                ", \"dateNaissEtu\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"lieuNaissEtu\": \"SAMPLE_TEXT\"" +
                                ", \"sexe\": \"SAMPLE_TEXT\"" +
                                ", \"numDocIdentite\": \"SAMPLE_TEXT\"" +
                                ", \"assimileYN\": null" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_etudiant_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created etudiant").get("/services/microservicegir${new_etudiant_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created etudiant")
                        .delete("/services/microservicegir${new_etudiant_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Etudiant entity").exec(scn);
