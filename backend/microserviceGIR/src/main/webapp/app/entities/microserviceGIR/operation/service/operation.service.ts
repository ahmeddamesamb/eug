...
type RestOf<T extends IOperation | NewOperation> = Omit<T, 'dateExecution'> & {
...
export class OperationService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/operations', 'microservicegir');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/operations/_search', 'microservicegir');
  ...
  protected convertDateFromClient<T extends IOperation | NewOperation | PartialUpdateOperation>(operation: T): RestOf<T> {
    return {
      ...operation,
      dateExecution: operation.dateExecution?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOperation: RestOperation): IOperation {
    return {
      ...restOperation,
      dateExecution: restOperation.dateExecution ? dayjs(restOperation.dateExecution) : undefined,
    };
  }
