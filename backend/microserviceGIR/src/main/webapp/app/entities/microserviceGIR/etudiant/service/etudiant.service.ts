...
type RestOf<T extends IEtudiant | NewEtudiant> = Omit<T, 'dateNaissEtu'> & {
  dateNaissEtu?: string | null;
};
...
export class EtudiantService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/etudiants', 'microservicegir');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/etudiants/_search', 'microservicegir');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestEtudiant[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),
      catchError(() => scheduled([new HttpResponse<IEtudiant[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends IEtudiant | NewEtudiant | PartialUpdateEtudiant>(etudiant: T): RestOf<T> {
    return {
      ...etudiant,
      dateNaissEtu: etudiant.dateNaissEtu?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEtudiant: RestEtudiant): IEtudiant {
    return {
      ...restEtudiant,
      dateNaissEtu: restEtudiant.dateNaissEtu ? dayjs(restEtudiant.dateNaissEtu) : undefined,
    };
  }
