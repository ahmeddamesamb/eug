...
import { EntityArrayResponseType, AnneeAcademiqueService } from '../service/annee-academique.service';
import { AnneeAcademiqueDeleteDialogComponent } from '../delete/annee-academique-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-annee-academique',
...
export class AnneeAcademiqueComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['libelleAnneeAcademique', 'separateur'];
  ...
  trackId = (_index: number, item: IAnneeAcademique): number => this.anneeAcademiqueService.getAnneeAcademiqueIdentifier(item);

  search(query: string): void {
    if (query && AnneeAcademiqueComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (AnneeAcademiqueComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.anneeAcademiques = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IAnneeAcademique[] | null): IAnneeAcademique[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.anneeAcademiqueService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.anneeAcademiqueService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
