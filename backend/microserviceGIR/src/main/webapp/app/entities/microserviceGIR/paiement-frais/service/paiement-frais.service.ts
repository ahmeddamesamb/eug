...
type RestOf<T extends IPaiementFrais | NewPaiementFrais> = Omit<T, 'datePaiement' | 'dateForclos'> & {
  datePaiement?: string | null;
  dateForclos?: string | null;
};
...
export class PaiementFraisService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/paiement-frais/_search', 'microservicegir');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestPaiementFrais[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),
      catchError(() => scheduled([new HttpResponse<IPaiementFrais[]>()], asapScheduler)),
    );
  }
  ...
  protected convertDateFromClient<T extends IPaiementFrais | NewPaiementFrais | PartialUpdatePaiementFrais>(paiementFrais: T): RestOf<T> {
    return {
      ...
      dateForclos: paiementFrais.dateForclos?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPaiementFrais: RestPaiementFrais): IPaiementFrais {
    return {
      ...
      dateForclos: restPaiementFrais.dateForclos ? dayjs(restPaiementFrais.dateForclos) : undefined,
    };
  }
