...
import { EntityArrayResponseType, CampagneService } from '../service/campagne.service';
import { CampagneDeleteDialogComponent } from '../delete/campagne-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-campagne',
...
export class CampagneComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['libelleCampagne', 'libelleAbrege'];
  ...
  trackId = (_index: number, item: ICampagne): number => this.campagneService.getCampagneIdentifier(item);

  search(query: string): void {
    if (query && CampagneComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (CampagneComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.campagnes = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ICampagne[] | null): ICampagne[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.campagneService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.campagneService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
