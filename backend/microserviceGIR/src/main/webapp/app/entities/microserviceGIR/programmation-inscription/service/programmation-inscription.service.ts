...
type RestOf<T extends IProgrammationInscription | NewProgrammationInscription> = Omit<
  T,
  'dateDebutProgrammation' | 'dateFinProgrammation' | 'dateForclosClasse'
> & {
  dateDebutProgrammation?: string | null;
  dateFinProgrammation?: string | null;
  dateForclosClasse?: string | null;
};
...
export class ProgrammationInscriptionService {
  ...
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/programmation-inscriptions/_search', 'microservicegir');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http.get<RestProgrammationInscription[]>(this.resourceSearchUrl, { params: options, observe: 'response' }).pipe(
      map(res => this.convertResponseArrayFromServer(res)),
      catchError(() => scheduled([new HttpResponse<IProgrammationInscription[]>()], asapScheduler)),
    );
  }
  ...
  >(programmationInscription: T): RestOf<T> {
    return {
      ...
      dateDebutProgrammation: programmationInscription.dateDebutProgrammation?.format(DATE_FORMAT) ?? null,
      dateFinProgrammation: programmationInscription.dateFinProgrammation?.format(DATE_FORMAT) ?? null,
      dateForclosClasse: programmationInscription.dateForclosClasse?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProgrammationInscription: RestProgrammationInscription): IProgrammationInscription {
    return {
      ...
      dateDebutProgrammation: restProgrammationInscription.dateDebutProgrammation
        ? dayjs(restProgrammationInscription.dateDebutProgrammation)
        : undefined,
      dateFinProgrammation: restProgrammationInscription.dateFinProgrammation
        ? dayjs(restProgrammationInscription.dateFinProgrammation)
        : undefined,
      dateForclosClasse: restProgrammationInscription.dateForclosClasse ? dayjs(restProgrammationInscription.dateForclosClasse) : undefined,
    };
  }
