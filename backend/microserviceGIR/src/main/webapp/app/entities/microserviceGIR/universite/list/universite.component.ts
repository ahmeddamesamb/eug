...
import { EntityArrayResponseType, UniversiteService } from '../service/universite.service';
import { UniversiteDeleteDialogComponent } from '../delete/universite-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-universite',
...
export class UniversiteComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['nomUniversite', 'sigleUniversite'];
  ...
  search(query: string): void {
    if (query && UniversiteComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  delete(universite: IUniversite): void {
    const modalRef = this.modalService.open(UniversiteDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (UniversiteComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.universites = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IUniversite[] | null): IUniversite[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.universiteService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.universiteService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
