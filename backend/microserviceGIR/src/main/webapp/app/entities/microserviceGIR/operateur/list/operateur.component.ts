...
import { EntityArrayResponseType, OperateurService } from '../service/operateur.service';
import { OperateurDeleteDialogComponent } from '../delete/operateur-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-operateur',
...
export class OperateurComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['libelleOperateur', 'userLogin', 'codeOperateur'];
  ...
  trackId = (_index: number, item: IOperateur): number => this.operateurService.getOperateurIdentifier(item);

  search(query: string): void {
    if (query && OperateurComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (OperateurComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.operateurs = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IOperateur[] | null): IOperateur[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.operateurService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.operateurService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
