...
public class MinistereResource {
    ...
    /**
     * {@code SEARCH  /ministeres/_search?query=:query} : search for the ministere corresponding
     * to the query.
     *
     * @param query the query of the ministere search.
     * @param pageable the pagination information.
    ...
    public ResponseEntity<List<MinistereDTO>> searchMinisteres(
        ...
        try {
            Page<MinistereDTO> page = ministereService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
