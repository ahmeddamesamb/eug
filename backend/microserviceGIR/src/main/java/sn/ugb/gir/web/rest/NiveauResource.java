...
/**
 * REST controller for managing {@link sn.ugb.gir.domain.Niveau}.
 */
@RestController
...
public class NiveauResource {
    ...
    private static final String ENTITY_NAME = "microserviceGirNiveau";
    ...
    /**
     * {@code SEARCH  /niveaus/_search?query=:query} : search for the niveau corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search")
    public ResponseEntity<List<NiveauDTO>> searchNiveaus(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Niveaus for query {}", query);
        try {
            Page<NiveauDTO> page = niveauService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
