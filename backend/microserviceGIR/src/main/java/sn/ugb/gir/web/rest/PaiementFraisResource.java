...
public class PaiementFraisResource {
    ...
    /**
     * {@code SEARCH  /paiement-frais/_search?query=:query} : search for the paiementFrais corresponding
     * to the query.
     *
     * @param query the query of the paiementFrais search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<PaiementFraisDTO>> searchPaiementFrais(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of PaiementFrais for query {}", query);
        try {
            Page<PaiementFraisDTO> page = paiementFraisService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
