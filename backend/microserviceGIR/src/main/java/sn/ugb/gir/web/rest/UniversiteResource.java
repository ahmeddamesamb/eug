...
public class UniversiteResource {
    ...
    /**
     * {@code SEARCH  /universites/_search?query=:query} : search for the universite corresponding
     * to the query.
     *
     * @param query the query of the universite search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<UniversiteDTO>> searchUniversites(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Universites for query {}", query);
        try {
            Page<UniversiteDTO> page = universiteService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
