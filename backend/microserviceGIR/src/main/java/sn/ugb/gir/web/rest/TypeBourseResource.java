...
public class TypeBourseResource {
    ...
    /**
     * {@code SEARCH  /type-bourses/_search?query=:query} : search for the typeBourse corresponding
     * to the query.
     *
     * @param query the query of the typeBourse search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<TypeBourseDTO>> searchTypeBourses(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of TypeBourses for query {}", query);
        try {
            Page<TypeBourseDTO> page = typeBourseService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
