...
public class TypeSelectionResource {
    ...
    /**
     * {@code SEARCH  /type-selections/_search?query=:query} : search for the typeSelection corresponding
     * to the query.
     *
     * @param query the query of the typeSelection search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<TypeSelectionDTO>> searchTypeSelections(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of TypeSelections for query {}", query);
        try {
            Page<TypeSelectionDTO> page = typeSelectionService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
