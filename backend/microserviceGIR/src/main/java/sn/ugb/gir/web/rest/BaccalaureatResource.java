...
public class BaccalaureatResource {
    ...
    /**
     * {@code SEARCH  /baccalaureats/_search?query=:query} : search for the baccalaureat corresponding
     * to the query.
     *
     * @param query the query of the baccalaureat search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<BaccalaureatDTO>> searchBaccalaureats(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Baccalaureats for query {}", query);
        try {
            Page<BaccalaureatDTO> page = baccalaureatService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
