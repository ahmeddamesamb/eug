...
public class OperateurResource {
    ...
    /**
     * {@code SEARCH  /operateurs/_search?query=:query} : search for the operateur corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search")
    public ResponseEntity<List<OperateurDTO>> searchOperateurs(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Operateurs for query {}", query);
        try {
            Page<OperateurDTO> page = operateurService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
