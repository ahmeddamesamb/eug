...
public class PaysResource {
    ...
    /**
     * {@code SEARCH  /pays/_search?query=:query} : search for the pays corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search")
    public ResponseEntity<List<PaysDTO>> searchPays(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Pays for query {}", query);
        try {
            Page<PaysDTO> page = paysService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
