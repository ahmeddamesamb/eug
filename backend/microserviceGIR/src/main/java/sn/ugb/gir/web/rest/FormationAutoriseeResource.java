...
public class FormationAutoriseeResource {
    ...
    /**
     * {@code SEARCH  /formation-autorisees/_search?query=:query} : search for the formationAutorisee corresponding
     * to the query.
     *
     * @param query the query of the formationAutorisee search.
     * @param pageable the pagination information.
    ...
    public ResponseEntity<List<FormationAutoriseeDTO>> searchFormationAutorisees(
        ...
        try {
            Page<FormationAutoriseeDTO> page = formationAutoriseeService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
