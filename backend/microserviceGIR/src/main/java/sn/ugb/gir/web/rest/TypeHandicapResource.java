...
public class TypeHandicapResource {
    ...
    /**
     * {@code SEARCH  /type-handicaps/_search?query=:query} : search for the typeHandicap corresponding
     * to the query.
     *
     * @param query the query of the typeHandicap search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<TypeHandicapDTO>> searchTypeHandicaps(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of TypeHandicaps for query {}", query);
        try {
            Page<TypeHandicapDTO> page = typeHandicapService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
