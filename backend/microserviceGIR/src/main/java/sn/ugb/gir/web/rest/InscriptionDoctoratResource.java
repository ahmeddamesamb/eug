...
public class InscriptionDoctoratResource {
    ...
    /**
     * {@code SEARCH  /inscription-doctorats/_search?query=:query} : search for the inscriptionDoctorat corresponding
     * to the query.
     *
     * @param query the query of the inscriptionDoctorat search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<InscriptionDoctoratDTO>> searchInscriptionDoctorats(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of InscriptionDoctorats for query {}", query);
        try {
            Page<InscriptionDoctoratDTO> page = inscriptionDoctoratService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
