...
public class TypeOperationResource {
    ...
    /**
     * {@code SEARCH  /type-operations/_search?query=:query} : search for the typeOperation corresponding
     * to the query.
     *
     ...
     */
    @GetMapping("/_search")
    public ResponseEntity<List<TypeOperationDTO>> searchTypeOperations(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of TypeOperations for query {}", query);
        try {
            Page<TypeOperationDTO> page = typeOperationService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
