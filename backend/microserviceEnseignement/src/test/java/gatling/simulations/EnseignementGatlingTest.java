...
/**
 * Performance test for the Enseignement entity.
 */
public class EnseignementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all enseignements")
                    .get("/services/microserviceenseignement/api/enseignements")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new enseignement")
                        .post("/services/microserviceenseignement/api/enseignements")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleEnseignements\": \"SAMPLE_TEXT\"" +
                                ", \"volumeHoraire\": 0" +
                                ", \"nombreInscrits\": 0" +
                                ", \"groupeYN\": null" +
                                "}"
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_enseignement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created enseignement")
                            .get("/services/microserviceenseignement${new_enseignement_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created enseignement")
                        .delete("/services/microserviceenseignement${new_enseignement_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Enseignement entity").exec(scn);
