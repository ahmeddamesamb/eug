...
class EnseignementResourceIT {
    ...
    private static final Boolean DEFAULT_GROUPE_YN = false;
    private static final Boolean UPDATED_GROUPE_YN = true;

    private static final String ENTITY_API_URL = "/api/enseignements";
    private static final String ENTITY_API_URL_ID = ENTITY_API_URL + "/{id}";
    private static final String ENTITY_SEARCH_API_URL = "/api/enseignements/_search";
    ...
    @Autowired
    private EnseignementSearchRepository enseignementSearchRepository;
    ...
    public void cleanupElasticSearchRepository() {
        enseignementSearchRepository.deleteAll();
        assertThat(enseignementSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createEnseignement() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(enseignementSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(enseignementSearchRepository.findAll());
    ...
    void getAllEnseignements() throws Exception {
        ...
        restEnseignementMockMvc
            ...
            .andExpect(jsonPath("$.[*].groupeYN").value(hasItem(DEFAULT_GROUPE_YN.booleanValue())));
    }
    ...
    void getEnseignement() throws Exception {
        ...
        restEnseignementMockMvc
            ...
            .andExpect(jsonPath("$.groupeYN").value(DEFAULT_GROUPE_YN.booleanValue()));
    }
    ...
    void putExistingEnseignement() throws Exception {
        ...
        enseignementSearchRepository.save(enseignement);
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(enseignementSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(enseignementSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<Enseignement> enseignementSearchList = IterableUtils.toList(enseignementSearchRepository.findAll());
                Enseignement testEnseignementSearch = enseignementSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testEnseignementSearch.getLibelleEnseignements()).isEqualTo(UPDATED_LIBELLE_ENSEIGNEMENTS);
                assertThat(testEnseignementSearch.getVolumeHoraire()).isEqualTo(UPDATED_VOLUME_HORAIRE);
                assertThat(testEnseignementSearch.getNombreInscrits()).isEqualTo(UPDATED_NOMBRE_INSCRITS);
                assertThat(testEnseignementSearch.getGroupeYN()).isEqualTo(UPDATED_GROUPE_YN);
            });
    }
    ...
    @Test
    @Transactional
    void searchEnseignement() throws Exception {
        // Initialize the database
        enseignement = enseignementRepository.saveAndFlush(enseignement);
        enseignementSearchRepository.save(enseignement);

        // Search the enseignement
        restEnseignementMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + enseignement.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(enseignement.getId().intValue())))
            .andExpect(jsonPath("$.[*].libelleEnseignements").value(hasItem(DEFAULT_LIBELLE_ENSEIGNEMENTS)))
