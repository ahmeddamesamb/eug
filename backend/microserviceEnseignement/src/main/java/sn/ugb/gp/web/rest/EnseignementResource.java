...
import sn.ugb.gp.web.rest.errors.ElasticsearchExceptionMapper;
...
public class EnseignementResource {
    ...
    /**
     * {@code SEARCH  /enseignements/_search?query=:query} : search for the enseignement corresponding
     * to the query.
     *
     * @param query the query of the enseignement search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<EnseignementDTO>> searchEnseignements(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of Enseignements for query {}", query);
        try {
            Page<EnseignementDTO> page = enseignementService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
