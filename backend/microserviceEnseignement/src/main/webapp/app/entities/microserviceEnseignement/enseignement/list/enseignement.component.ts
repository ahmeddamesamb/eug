...
import { EntityArrayResponseType, EnseignementService } from '../service/enseignement.service';
import { EnseignementDeleteDialogComponent } from '../delete/enseignement-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-enseignement',
...
export class EnseignementComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['libelleEnseignements'];
  ...
  trackId = (_index: number, item: IEnseignement): number => this.enseignementService.getEnseignementIdentifier(item);

  search(query: string): void {
    if (query && EnseignementComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (EnseignementComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.enseignements = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IEnseignement[] | null): IEnseignement[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.enseignementService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.enseignementService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
