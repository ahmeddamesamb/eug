...
import { EntityArrayResponseType, PlanningService } from '../service/planning.service';
import { PlanningDeleteDialogComponent } from '../delete/planning-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-planning',
...
export class PlanningComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPlanning): number => this.planningService.getPlanningIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.plannings = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IPlanning[] | null): IPlanning[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.planningService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.planningService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
