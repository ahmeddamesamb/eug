...
/**
 * Performance test for the Planning entity.
 */
public class PlanningGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all plannings")
                    .get("/services/microserviceedt/api/plannings")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new planning")
                        .post("/services/microserviceedt/api/plannings")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" + "\"dateDebut\": \"2020-01-01T00:00:00.000Z\"" + ", \"dateFin\": \"2020-01-01T00:00:00.000Z\"" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_planning_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created planning").get("/services/microserviceedt${new_planning_url}").headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created planning")
                        .delete("/services/microserviceedt${new_planning_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Planning entity").exec(scn);
