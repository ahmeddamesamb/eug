...
/**
 * Performance test for the Enseignant entity.
 */
public class EnseignantGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8086");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all enseignants")
                    .get("/services/microservicegrh/api/enseignants")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new enseignant")
                        .post("/services/microservicegrh/api/enseignants")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"titreCoEncadreur\": \"SAMPLE_TEXT\"" +
                                ", \"nom\": \"SAMPLE_TEXT\"" +
                                ", \"prenom\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_enseignant_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created enseignant")
                            .get("/services/microservicegrh${new_enseignant_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created enseignant")
                        .delete("/services/microservicegrh${new_enseignant_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Enseignant entity").exec(scn);
