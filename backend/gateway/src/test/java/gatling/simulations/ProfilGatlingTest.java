...
/**
 * Performance test for the Profil entity.
 */
public class ProfilGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all profils").get("/api/profils").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new profil")
                        .post("/api/profils")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_profil_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created profil").get("${new_profil_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created profil").delete("${new_profil_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Profil entity").exec(scn);
