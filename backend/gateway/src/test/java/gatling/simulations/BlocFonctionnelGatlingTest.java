...
/**
 * Performance test for the BlocFonctionnel entity.
 */
public class BlocFonctionnelGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all blocFonctionnels").get("/api/bloc-fonctionnels").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new blocFonctionnel")
                        .post("/api/bloc-fonctionnels")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleBloc\": \"SAMPLE_TEXT\"" +
                                ", \"dateAjoutBloc\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_blocFonctionnel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created blocFonctionnel").get("${new_blocFonctionnel_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created blocFonctionnel").delete("${new_blocFonctionnel_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BlocFonctionnel entity").exec(scn);
