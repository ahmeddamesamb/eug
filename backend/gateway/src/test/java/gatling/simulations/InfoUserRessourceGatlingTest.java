...
/**
 * Performance test for the InfoUserRessource entity.
 */
public class InfoUserRessourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all infoUserRessources")
                    .get("/api/info-user-ressources")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new infoUserRessource")
                        .post("/api/info-user-ressources")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"dateAjout\": \"2020-01-01T00:00:00.000Z\"" + ", \"enCoursYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_infoUserRessource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created infoUserRessource").get("${new_infoUserRessource_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(http("Delete created infoUserRessource").delete("${new_infoUserRessource_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InfoUserRessource entity").exec(scn);
