...
/**
 * Performance test for the HistoriqueConnexion entity.
 */
public class HistoriqueConnexionGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all historiqueConnexions")
                ...
                .exec(
                    http("Create new historiqueConnexion")
                        .post("/api/historique-connexions")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"dateDebutConnexion\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"dateFinConnexion\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"adresseIp\": \"SAMPLE_TEXT\"" +
                                ", \"actifYN\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_historiqueConnexion_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created historiqueConnexion").get("${new_historiqueConnexion_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created historiqueConnexion").delete("${new_historiqueConnexion_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HistoriqueConnexion entity").exec(scn);
