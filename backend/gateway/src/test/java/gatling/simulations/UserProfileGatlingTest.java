...
public class UserProfileGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all userProfiles").get("/api/user-profiles").headers(headers_http_authenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created userProfile").get("${new_userProfile_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created userProfile").delete("${new_userProfile_url}").headers(headers_http_authenticated))
                .pause(10)
        );
