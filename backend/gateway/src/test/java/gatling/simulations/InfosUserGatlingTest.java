...
/**
 * Performance test for the InfosUser entity.
 */
public class InfosUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all infosUsers").get("/api/infos-users").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new infosUser")
                        .post("/api/infos-users")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"dateAjout\": \"2020-01-01T00:00:00.000Z\"" + ", \"actifYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_infosUser_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created infosUser").get("${new_infosUser_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created infosUser").delete("${new_infosUser_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InfosUser entity").exec(scn);
