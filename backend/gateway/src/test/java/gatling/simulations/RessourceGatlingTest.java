...
/**
 * Performance test for the Ressource entity.
 */
public class RessourceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all ressources").get("/api/ressources").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ressource")
                        .post("/api/ressources")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelle\": \"SAMPLE_TEXT\"" + ", \"actifYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ressource_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ressource").get("${new_ressource_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created ressource").delete("${new_ressource_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Ressource entity").exec(scn);
