...
/**
 * Performance test for the ServiceUser entity.
 */
public class ServiceUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all serviceUsers").get("/api/service-users").headers(headers_http_authenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new serviceUser")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_serviceUser_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created serviceUser").get("${new_serviceUser_url}").headers(headers_http_authenticated)).pause(10))
                .exec(http("Delete created serviceUser").delete("${new_serviceUser_url}").headers(headers_http_authenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ServiceUser entity").exec(scn);
