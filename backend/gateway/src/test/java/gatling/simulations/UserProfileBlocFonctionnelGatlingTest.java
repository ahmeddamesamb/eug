...
/**
 * Performance test for the UserProfileBlocFonctionnel entity.
 */
public class UserProfileBlocFonctionnelGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all userProfileBlocFonctionnels")
                    .get("/api/user-profile-bloc-fonctionnels")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userProfileBlocFonctionnel")
                        .post("/api/user-profile-bloc-fonctionnels")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"date\": \"2020-01-01T00:00:00.000Z\"" + ", \"enCoursYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userProfileBlocFonctionnel_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created userProfileBlocFonctionnel")
                            .get("${new_userProfileBlocFonctionnel_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created userProfileBlocFonctionnel")
                        .delete("${new_userProfileBlocFonctionnel_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserProfileBlocFonctionnel entity").exec(scn);
