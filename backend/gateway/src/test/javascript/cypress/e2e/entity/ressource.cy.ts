...
describe('Ressource e2e test', () => {
  ...
  const ressourceSample = { libelle: 'bientôt afin que de façon à', actifYN: true };
        ...
        cy.authenticatedRequest({
          ...
          cy.intercept(
            ...
            },
            {
              statusCode: 200,
              headers: {
                link: '<http://localhost/api/ressources?page=0&size=20>; rel="last",<http://localhost/api/ressources?page=0&size=20>; rel="first"',
              },
              body: [ressource],
            },
  ...
  describe('new Ressource page', () => {
    ...
    it('should create an instance of Ressource', () => {
      cy.get(`[data-cy="libelle"]`).type('réagir');
      cy.get(`[data-cy="libelle"]`).should('have.value', 'réagir');
