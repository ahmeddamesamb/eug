...
export class RessourceComponent implements OnInit {
  ...
  search(query: string): void {
    if (query && RessourceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  delete(ressource: IRessource): void {
    const modalRef = this.modalService.open(RessourceDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (RessourceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.ressources = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IRessource[] | null): IRessource[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.ressourceService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.ressourceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
