...
class DeliberationResourceIT {
    ...
    private static final Boolean DEFAULT_EST_VALIDEE_YN = false;
    private static final Boolean UPDATED_EST_VALIDEE_YN = true;
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/deliberations/_search";
    ...
    public void cleanupElasticSearchRepository() {
        ...
        assertThat(deliberationSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createDeliberation() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(deliberationSearchRepository.findAll());
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(deliberationSearchRepository.findAll());
    ...
    void getAllDeliberations() throws Exception {
        ...
        restDeliberationMockMvc
            ...
            .andExpect(jsonPath("$.[*].estValideeYN").value(hasItem(DEFAULT_EST_VALIDEE_YN.booleanValue())))
    ...
    void getDeliberation() throws Exception {
        ...
        restDeliberationMockMvc
            ...
            .andExpect(jsonPath("$.estValideeYN").value(DEFAULT_EST_VALIDEE_YN.booleanValue()))
    ...
    void putExistingDeliberation() throws Exception {
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(deliberationSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<Deliberation> deliberationSearchList = IterableUtils.toList(deliberationSearchRepository.findAll());
                Deliberation testDeliberationSearch = deliberationSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testDeliberationSearch.getEstValideeYN()).isEqualTo(UPDATED_EST_VALIDEE_YN);
                assertThat(testDeliberationSearch.getPvDeliberation()).isEqualTo(UPDATED_PV_DELIBERATION);
                assertThat(testDeliberationSearch.getPvDeliberationContentType()).isEqualTo(UPDATED_PV_DELIBERATION_CONTENT_TYPE);
            });
    }
    ...
    @Test
    @Transactional
    void searchDeliberation() throws Exception {
        // Initialize the database
        deliberation = deliberationRepository.saveAndFlush(deliberation);
        deliberationSearchRepository.save(deliberation);

        // Search the deliberation
        restDeliberationMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + deliberation.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(deliberation.getId().intValue())))
            .andExpect(jsonPath("$.[*].estValideeYN").value(hasItem(DEFAULT_EST_VALIDEE_YN.booleanValue())))
