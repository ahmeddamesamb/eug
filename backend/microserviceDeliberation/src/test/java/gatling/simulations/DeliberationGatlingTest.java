...
/**
 * Performance test for the Deliberation entity.
 */
public class DeliberationGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8088");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all deliberations")
                    .get("/services/microservicedeliberation/api/deliberations")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new deliberation")
                        .post("/services/microservicedeliberation/api/deliberations")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"estValideeYN\": null" + ", \"pvDeliberation\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_deliberation_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created deliberation")
                            .get("/services/microservicedeliberation${new_deliberation_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created deliberation")
                        .delete("/services/microservicedeliberation${new_deliberation_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Deliberation entity").exec(scn);
