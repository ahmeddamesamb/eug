...
import { EntityArrayResponseType, DeliberationService } from '../service/deliberation.service';
import { DeliberationDeleteDialogComponent } from '../delete/deliberation-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-deliberation',
...
export class DeliberationComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['pvDeliberation'];
  ...
  trackId = (_index: number, item: IDeliberation): number => this.deliberationService.getDeliberationIdentifier(item);

  search(query: string): void {
    if (query && DeliberationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (DeliberationComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.deliberations = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IDeliberation[] | null): IDeliberation[] {
    return data ?? [];
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.deliberationService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
      return this.deliberationService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
  }
