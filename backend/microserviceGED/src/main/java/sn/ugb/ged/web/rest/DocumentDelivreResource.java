...
public class DocumentDelivreResource {
    ...
    /**
     * {@code SEARCH  /document-delivres/_search?query=:query} : search for the documentDelivre corresponding
     * to the query.
     *
     * @param query the query of the documentDelivre search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DocumentDelivreDTO>> searchDocumentDelivres(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of DocumentDelivres for query {}", query);
        try {
            Page<DocumentDelivreDTO> page = documentDelivreService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
