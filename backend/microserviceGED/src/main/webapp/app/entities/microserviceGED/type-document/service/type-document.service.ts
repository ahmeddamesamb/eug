...
export class TypeDocumentService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/type-documents', 'microserviceged');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/type-documents/_search', 'microserviceged');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<ITypeDocument[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(catchError(() => scheduled([new HttpResponse<ITypeDocument[]>()], asapScheduler)));
  }
