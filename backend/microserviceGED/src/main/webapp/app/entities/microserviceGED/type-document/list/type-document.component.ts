...
import { EntityArrayResponseType, TypeDocumentService } from '../service/type-document.service';
import { TypeDocumentDeleteDialogComponent } from '../delete/type-document-delete-dialog.component';

@Component({
  standalone: true,
  selector: 'ugb-type-document',
...
export class TypeDocumentComponent implements OnInit {
  private static readonly NOT_SORTABLE_FIELDS_AFTER_SEARCH = ['libelleTypeDocument'];
  ...
  trackId = (_index: number, item: ITypeDocument): number => this.typeDocumentService.getTypeDocumentIdentifier(item);

  search(query: string): void {
    if (query && TypeDocumentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (TypeDocumentComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected fillComponentAttributesFromResponseBody(data: ITypeDocument[] | null): ITypeDocument[] {
    return data ?? [];
  }
