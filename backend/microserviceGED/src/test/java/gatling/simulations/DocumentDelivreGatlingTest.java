...
/**
 * Performance test for the DocumentDelivre entity.
 */
public class DocumentDelivreGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8092");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all documentDelivres")
                    .get("/services/microserviceged/api/document-delivres")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new documentDelivre")
                        .post("/services/microserviceged/api/document-delivres")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleDoc\": \"SAMPLE_TEXT\"" +
                                ", \"anneeDoc\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_documentDelivre_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created documentDelivre")
                            .get("/services/microserviceged${new_documentDelivre_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created documentDelivre")
                        .delete("/services/microserviceged${new_documentDelivre_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the DocumentDelivre entity").exec(scn);
