 ...
 */
public class TypeDocumentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeDocuments")
                    .get("/services/microserviceged/api/type-documents")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeDocument")
                        .post("/services/microserviceged/api/type-documents")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeDocument\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeDocument_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeDocument")
                            .get("/services/microserviceged${new_typeDocument_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeDocument")
                        .delete("/services/microserviceged${new_typeDocument_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeDocument entity").exec(scn);
