...
public class TypeRapportResource {
    ...
    /**
     * {@code SEARCH  /type-rapports/_search?query=:query} : search for the typeRapport corresponding
     * to the query.
     *
     * @param query the query of the typeRapport search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<TypeRapportDTO>> searchTypeRapports(
        @RequestParam("query") String query,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        log.debug("REST request to search for a page of TypeRapports for query {}", query);
        try {
            Page<TypeRapportDTO> page = typeRapportService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
