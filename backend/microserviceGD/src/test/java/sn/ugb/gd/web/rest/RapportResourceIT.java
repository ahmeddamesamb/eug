...
class RapportResourceIT {
    ...
    private static final String ENTITY_SEARCH_API_URL = "/api/rapports/_search";
    ...
    @Autowired
    private RapportSearchRepository rapportSearchRepository;
    ...
    public void cleanupElasticSearchRepository() {
        rapportSearchRepository.deleteAll();
        assertThat(rapportSearchRepository.count()).isEqualTo(0);
    }
    ...
    void createRapport() throws Exception {
        ...
        int searchDatabaseSizeBefore = IterableUtil.sizeOf(rapportSearchRepository.findAll());
        // Create the Rapport
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(rapportSearchRepository.findAll());
    ...
    void putExistingRapport() throws Exception {
        ...
        await()
            ...
            .untilAsserted(() -> {
                int searchDatabaseSizeAfter = IterableUtil.sizeOf(rapportSearchRepository.findAll());
                assertThat(searchDatabaseSizeAfter).isEqualTo(searchDatabaseSizeBefore);
                List<Rapport> rapportSearchList = IterableUtils.toList(rapportSearchRepository.findAll());
                Rapport testRapportSearch = rapportSearchList.get(searchDatabaseSizeAfter - 1);
                assertThat(testRapportSearch.getLibelleRapport()).isEqualTo(UPDATED_LIBELLE_RAPPORT);
                assertThat(testRapportSearch.getDescriptionRapport()).isEqualTo(UPDATED_DESCRIPTION_RAPPORT);
                assertThat(testRapportSearch.getContenuRapport()).isEqualTo(UPDATED_CONTENU_RAPPORT);
                assertThat(testRapportSearch.getDateRedaction()).isEqualTo(UPDATED_DATE_REDACTION);
            });
    }
    ...
    @Test
    @Transactional
    void searchRapport() throws Exception {
        // Initialize the database
        rapport = rapportRepository.saveAndFlush(rapport);
        rapportSearchRepository.save(rapport);

        // Search the rapport
        restRapportMockMvc
            .perform(get(ENTITY_SEARCH_API_URL + "?query=id:" + rapport.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(rapport.getId().intValue())))
            .andExpect(jsonPath("$.[*].libelleRapport").value(hasItem(DEFAULT_LIBELLE_RAPPORT)))
