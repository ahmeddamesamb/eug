...
/**
 * Performance test for the TypeRapport entity.
 */
public class TypeRapportGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all typeRapports")
                    .get("/services/microservicegd/api/type-rapports")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new typeRapport")
                        .post("/services/microservicegd/api/type-rapports")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"libelleTypeRapport\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_typeRapport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created typeRapport")
                            .get("/services/microservicegd${new_typeRapport_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created typeRapport")
                        .delete("/services/microservicegd${new_typeRapport_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TypeRapport entity").exec(scn);
