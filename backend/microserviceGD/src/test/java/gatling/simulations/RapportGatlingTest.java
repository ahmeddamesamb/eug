...
/**
 * Performance test for the Rapport entity.
 */
public class RapportGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8089");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all rapports")
                    .get("/services/microservicegd/api/rapports")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new rapport")
                        .post("/services/microservicegd/api/rapports")
                        .headers(headers_http_authenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"libelleRapport\": \"SAMPLE_TEXT\"" +
                                ", \"descriptionRapport\": \"SAMPLE_TEXT\"" +
                                ", \"contenuRapport\": \"SAMPLE_TEXT\"" +
                                ", \"dateRedaction\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_rapport_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created rapport").get("/services/microservicegd${new_rapport_url}").headers(headers_http_authenticated))
                        .pause(10)
                )
                .exec(
                    http("Delete created rapport").delete("/services/microservicegd${new_rapport_url}").headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Rapport entity").exec(scn);
