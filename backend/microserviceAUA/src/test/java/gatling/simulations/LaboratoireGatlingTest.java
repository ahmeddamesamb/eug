...
/**
 * Performance test for the Laboratoire entity.
 */
public class LaboratoireGatlingTest extends Simulation {
    ...
    String baseURL = Optional.ofNullable(System.getProperty("baseURL")).orElse("http://localhost:8087");
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all laboratoires")
                    .get("/services/microserviceaua/api/laboratoires")
                    .headers(headers_http_authenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new laboratoire")
                        .post("/services/microserviceaua/api/laboratoires")
                        .headers(headers_http_authenticated)
                        .body(StringBody("{" + "\"nom\": \"SAMPLE_TEXT\"" + ", \"laboratoireCotutelleYN\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_laboratoire_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created laboratoire")
                            .get("/services/microserviceaua${new_laboratoire_url}")
                            .headers(headers_http_authenticated)
                    )
                        .pause(10)
                )
                .exec(
                    http("Delete created laboratoire")
                        .delete("/services/microserviceaua${new_laboratoire_url}")
                        .headers(headers_http_authenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Laboratoire entity").exec(scn);
