...
export class LaboratoireComponent implements OnInit {
  ...
  trackId = (_index: number, item: ILaboratoire): number => this.laboratoireService.getLaboratoireIdentifier(item);

  search(query: string): void {
    if (query && LaboratoireComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
      this.predicate = 'id';
      this.ascending = true;
    }
    this.page = 1;
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      if (LaboratoireComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(this.predicate)) {
        this.predicate = '';
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.laboratoires = dataFromBody;
  }
  ...
  protected queryBackend(
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.laboratoireService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    } else {
