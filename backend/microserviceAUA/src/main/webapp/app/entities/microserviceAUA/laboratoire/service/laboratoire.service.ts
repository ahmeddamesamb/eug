...
export class LaboratoireService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/laboratoires', 'microserviceaua');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/laboratoires/_search', 'microserviceaua');
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<ILaboratoire[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(catchError(() => scheduled([new HttpResponse<ILaboratoire[]>()], asapScheduler)));
  }
